...
class IntelligentCommitAnalyzer:
                ...
                if line.strip():
                    ...
                    if len(parts) >= 3:
                        ...
                        file_change = FileChange(
                            ...
                            is_config=self._matches_patterns(file_path, self.CONFIG_PATTERNS),
    ...
    def analyze_commit_messages(self, messages: List[str]) -> Dict[str, Any]:
        ...
        analysis = {
            'conventional_commits': {
                ...
                'refactor': 0, 'perf': 0, 'test': 0, 'build': 0,
                'ci': 0, 'chore': 0, 'revert': 0
            },
            'breaking_keywords': 0,
            'feature_keywords': 0,
            'fix_keywords': 0,
        ...
        conventional_pattern = re.compile(r'^(?P<type>\w+)(?:\([^)]+\))?!?:\s*(?P<desc>.+)')
    ...
    def determine_version_bump(self, file_changes: List[FileChange],
                             content_analysis: Dict[str, Any],
                             message_analysis: Dict[str, Any]) -> ChangeType:
        """Determine version bump based on all analysis."""

        # Check for breaking changes (highest priority)
        breaking_indicators = (
            content_analysis['breaking_changes'] > 0 or
            message_analysis['breaking_keywords'] > 0 or
            any(change.is_api and change.change_type == 'D' for change in file_changes)
        )
        ...
        feature_indicators = (
            content_analysis['features'] > 0 or
            message_analysis['feature_keywords'] > 0 or
            message_analysis['conventional_commits']['feat'] > 0 or
            any(change.is_api and change.change_type == 'A' for change in file_changes)
        )
        ...
        # Check for bug fixes
        fix_indicators = (
            content_analysis['bug_fixes'] > 0 or
            message_analysis['fix_keywords'] > 0 or
            message_analysis['conventional_commits']['fix'] > 0 or
            any(change.is_api and change.change_type == 'M' for change in file_changes)
        )
        ...
        significant_changes = (
            any(change.is_config for change in file_changes) or
            any(change.is_api for change in file_changes)
        )
    ...
    def generate_intelligent_analysis(self, commit_range: str) -> Dict[str, Any]:
        ...
        version_bump = self.determine_version_bump(file_changes, content_analysis, message_analysis)
        ...
        categorized_changes = {
            ...
            'other_changes': [c for c in file_changes if not any([c.is_api, c.is_config, c.is_doc, c.is_test])]
        }
        ...
        return {
            ...
            'file_changes': {
                ...
                'files_modified': len([c for c in file_changes if c.change_type == 'M']),
            ...
            'categorized_changes': {
                category: [{'path': c.path, 'type': c.change_type, 'additions': c.additions, 'deletions': c.deletions}
                          for c in changes]
    ...
    def _generate_analysis_summary(self, file_changes: List[FileChange],
                                  content_analysis: Dict[str, Any],
                                  message_analysis: Dict[str, Any],
                                  version_bump: ChangeType) -> str:
        ...
        if content_analysis['breaking_changes'] > 0:
            summary_parts.append(f"{content_analysis['breaking_changes']} potential breaking change(s)")
        if content_analysis['features'] > 0:
            summary_parts.append(f"{content_analysis['features']} new feature(s) detected")
        if content_analysis['bug_fixes'] > 0:
            summary_parts.append(f"{content_analysis['bug_fixes']} bug fix(es) detected")

        return " | ".join(summary_parts)

    def _generate_recommendations(self, file_changes: List[FileChange],
                                 content_analysis: Dict[str, Any],
                                 message_analysis: Dict[str, Any],
                                 version_bump: ChangeType) -> List[str]:
        """Generate recommendations based on analysis."""
        recommendations = []

        if version_bump == ChangeType.MAJOR:
            recommendations.append("⚠️  Breaking changes detected - consider API versioning")
            ...
            recommendations.append("📝 Consider conventional commits for better tracking")

        # API-specific recommendations
        api_changes = [c for c in file_changes if c.is_api]
        if api_changes:
            recommendations.append(f"🔧 {len(api_changes)} API file(s) modified - review impact")

        # Message quality recommendations
        conventional_ratio = sum(message_analysis['conventional_commits'].values()) / max(message_analysis['total_messages'], 1)
...
def main():
    parser = argparse.ArgumentParser(description='Intelligent commit analysis for version bumping')
