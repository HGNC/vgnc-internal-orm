...
class VersionBumpDeterminer:
    ...
    def get_bump_reason(self, analysis_data: Dict[str, Any], bump_type: str) -> str:
        ...
        if bump_type == 'major':
            ...
            if breaking_count > 0:
                ...
                for breaking in categorized.get('breaking_changes', [])[:3]:  # Show first 3
            ...
            if not any(stats.get('conventional_commits', {}).values()):
                ...
                reasons.append("Only non-versionable changes (docs, style, refactoring, etc.)")
    ...
    def analyze_commit_types(self, analysis_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        conventional_commits = analysis_data.get('stats', {}).get('conventional_commits', {})
        ...
        version_impacts = {
            ...
            'minor_changes': ['feat'],
            'patch_changes': ['fix'],
            'no_version_changes': ['docs', 'style', 'refactor', 'perf', 'test', 'build', 'ci', 'chore', 'revert']
        }
            ...
            if count > 0:
                ...
                if found_impact:
                    impact_summary[found_impact]['commits'].append({
                        'type': commit_type,
                        'count': count,
                        'description': self._get_commit_type_description(commit_type)
                    })
...
def main():
    parser = argparse.ArgumentParser(description='Determine version bump from commit analysis')
    ...
    if args.format == 'json':
        output = {
            ...
            'summary': {
                ...
                'breaking_changes': analysis_data.get('stats', {}).get('breaking_changes', 0),
