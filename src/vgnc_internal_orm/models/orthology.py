...
class GeneFamilySpeciesEnhanced(BaseCustomModel):
    ...
    # Metadata fields
    gene_count: Mapped[int | None] = mapped_column(
        Integer,
        nullable=True,
        comment="Number of genes from this species in the family",
    )

    confidence_score: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="Confidence score for the assignment"
    )

    evidence_type: Mapped[str | None] = mapped_column(
        String(50), nullable=True, comment="Type of evidence supporting this assignment"
    )

    evidence_source: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="Source of the evidence"
    )

    curator_notes: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Notes from the curator"
    )
    ...
    # Relationships
    genefam: Mapped["Genefam"] = relationship("Genefam", lazy="joined")

    species: Mapped["Species"] = relationship("Species", lazy="joined")
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.date_assigned is None:
            self.date_assigned = func.now()  # type: ignore[assignment]
...
class GeneOrthologyGroup(BaseCustomModel):
    ...
    group_name: Mapped[str] = mapped_column(
        String(255), nullable=False, comment="Human-readable name for the group"
    )

    group_description: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Description of the orthology group"
    )

    # Metadata
    confidence_score: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="Overall confidence score for the group"
    )

    conservation_level: Mapped[str | None] = mapped_column(
        String(50),
        nullable=True,
        comment="Level of conservation (e.g., 'high', 'moderate', 'low')",
    )

    phylogenetic_scope: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="Phylogenetic scope of the group"
    )

    creation_method: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="Method used to create this group"
    )

    curator: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="Name of the curator who created this group"
    )
    ...
    def get_species_count(self) -> int:
        ...
        if not self.members:
            return 0
        return len({member.species_id for member in self.members})
    ...
    def get_species_ids(self) -> list[int]:
        ...
        return (
            list({member.species_id for member in self.members}) if self.members else []
        )
    ...
    def get_high_confidence_members(self) -> list["GeneFamilyGroupMember"]:
        ...
        for member in self.members:
            if (
                member.membership_confidence
                and float(member.membership_confidence) >= 0.8
            ):
    ...
    def find_by_confidence(
        cls, session: Session, min_confidence: float
    ) -> list["GeneOrthologyGroup"]:
    ...
    @classmethod
    def find_by_conservation(
        cls, session: Session, conservation_level: str
    ) -> list["GeneOrthologyGroup"]:
        """Find orthology groups by conservation level."""
        return (
            session.query(cls)
            .filter(cls.conservation_level == conservation_level, cls.is_active)
            .all()
        )
...
class GeneFamilyGroupMember(BaseCustomModel):
    ...
    # Membership metadata
    role_in_group: Mapped[str | None] = mapped_column(
        String(100), nullable=True, comment="Role of this gene family in the group"
    )

    membership_confidence: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="Confidence of this specific membership"
    )

    supporting_evidence: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Evidence supporting this membership"
    )
    ...
    added_by: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="Who added this member"
    )
    ...
    orthology_group: Mapped[GeneOrthologyGroup] = relationship(
        "GeneOrthologyGroup", back_populates="members", lazy="joined"
    )
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.date_added is None:
            self.date_added = func.now()  # type: ignore[assignment]
...
class SpeciesRelationship(BaseCustomModel):
    ...
    evolutionary_distance: Mapped[str | None] = mapped_column(
        String(20), nullable=True, comment="Evolutionary distance estimate"
    )

    divergence_time_mya: Mapped[str | None] = mapped_column(
        String(20), nullable=True, comment="Divergence time in million years ago"
    )

    synteny_score: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="Synteny conservation score"
    )

    genome_similarity: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="Overall genome similarity percentage"
    )

    ortholog_count: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Number of orthologs between species"
    )

    paralog_count: Mapped[int | None] = mapped_column(
        Integer, nullable=True, comment="Number of paralogs between species"
    )

    confidence_score: Mapped[str | None] = mapped_column(
        String(10), nullable=True, comment="Confidence score for the relationship"
    )

    evidence_source: Mapped[str | None] = mapped_column(
        String(255), nullable=True, comment="Source of the relationship data"
    )

    publication_reference: Mapped[str | None] = mapped_column(
        Text, nullable=True, comment="Reference publication"
    )
    ...
    date_established: Mapped[DateTime | None] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
    ...
    species_a: Mapped["Species"] = relationship(
        "Species", foreign_keys=[species_a_id], lazy="joined"
    )

    species_b: Mapped["Species"] = relationship(
        "Species", foreign_keys=[species_b_id], lazy="joined"
    )
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.date_established is None:
            self.date_established = func.now()  # type: ignore[assignment]
    ...
    # Helper methods
    def get_distance_as_float(self) -> float | None:
    ...
    def get_similarity_as_float(self) -> float | None:
    ...
    def get_divergence_time_as_float(self) -> float | None:
    ...
    @classmethod
    def find_between_species(
        cls, session: Session, species_a_id: int, species_b_id: int
    ) -> list["SpeciesRelationship"]:
        ...
        return (
            ...
            .filter(
                cls.is_active,
                (
                    (cls.species_a_id == species_a_id)
                    & (cls.species_b_id == species_b_id)
                    | (cls.species_a_id == species_b_id)
                    & (cls.species_b_id == species_a_id)
                ),
            )
            .all()
        )
    ...
    @classmethod
    def find_by_type(
        cls, session: Session, relationship_type: str
    ) -> list["SpeciesRelationship"]:
        """Find relationships by type."""
        return (
            session.query(cls)
            .filter(cls.relationship_type == relationship_type, cls.is_active)
            .all()
        )

    @classmethod
    def find_closest_species(
        cls, session: Session, species_id: int, limit: int = 5
    ) -> list["SpeciesRelationship"]:
        ...
        return (
            ...
            .filter(
                cls.is_active,
                ((cls.species_a_id == species_id) | (cls.species_b_id == species_id)),
            )
            .order_by(cls.evolutionary_distance.asc())
            .limit(limit)
            .all()
        )
