...
class SessionFactory:
    ...
    def _get_pool_config(self, is_async: bool = False) -> Dict[str, Any]:
        ...
        if self.config.driver.value.startswith("sqlite"):
            ...
            # PostgreSQL/MySQL use QueuePool for sync, NullPool for async
            if is_async:
    ...
    def _get_connect_args(self) -> Dict[str, Any]:
        ...
        elif self.config.driver.value.startswith("postgresql"):
            # PostgreSQL-specific configuration
            connect_args.update({
                "server_settings": {
                    "application_name": "vgnc_internal_orm",
                }
            })
    ...
    def _create_engine(self) -> Engine:
        ...
        if self.config.driver.value.startswith("postgresql") and self.config.ssl_mode:
            ssl_config = {"sslmode": self.config.ssl_mode}
            if self.config.ssl_cert:
                ssl_config["sslcert"] = str(self.config.ssl_cert)
            if self.config.ssl_key:
                ssl_config["sslkey"] = str(self.config.ssl_key)
            if self.config.ssl_ca:
                ssl_config["sslrootcert"] = str(self.config.ssl_ca)

            if "connect_args" not in engine_kwargs:
                engine_kwargs["connect_args"] = {}
            engine_kwargs["connect_args"].update(ssl_config)

        logger.info(f"Creating SQLAlchemy engine for {self.config.driver.value} "
    ...
    def _create_async_engine(self) -> AsyncEngine:
        ...
        if self.config is None:
            raise ValueError("Database configuration is required to create async engine")
    ...
    def _register_engine_events(self, engine: Engine) -> None:
        ...
        if hasattr(engine, 'dispatch') and not hasattr(engine, '_mock_name'):
            ...
            def receive_before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
                ...
                if self.config.echo and self.config.environment.value == "development":
    ...
    def get_engine_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "database_url": self.config.database_url.get_secret_value()[:50] + "...",  # Truncated for security
...
def get_async_session(config: Optional[DatabaseConfig] = None) -> AsyncSession:
...
@contextmanager
def get_session_context(config: Optional[DatabaseConfig] = None) -> Generator[Session, None, None]:
...
@asynccontextmanager
async def get_async_session_context(config: Optional[DatabaseConfig] = None) -> AsyncGenerator[AsyncSession, None]:
