...
class MigrationSafetyValidator:
    ...
    def _initialize_dangerous_patterns(self) -> List[Tuple[str, str, RiskLevel]]:
        ...
        return [
            ...
            (r'\bTRUNCATE\s+TABLE\b', 'Truncating table data', RiskLevel.CRITICAL),  # Raw SQL
            (r'\bdelete\s+from\b', 'Direct DELETE operation', RiskLevel.HIGH),
            (r'\bDELETE\s+FROM\b', 'Direct DELETE operation', RiskLevel.HIGH),  # Raw SQL
            ...
            (r'\balter_table.*change.*varchar.*to.*varchar\b', 'VARCHAR length reduction', RiskLevel.MEDIUM),
            (r'\balter_table.*modify.*varchar.*to.*varchar\b', 'VARCHAR length reduction', RiskLevel.MEDIUM),

            # Direct data manipulation
            (r'\bupdate.*set.*where.*1\s*=\s*1\b', 'UPDATE without WHERE clause', RiskLevel.CRITICAL),
            (r'\bdelete.*where.*1\s*=\s*1\b', 'DELETE without WHERE clause', RiskLevel.CRITICAL),
        ]
    ...
    def _extract_operation_type(self, line: str) -> str:
        ...
        if 'TABLE' in line.upper():
            # Extract table name
            match = re.search(r'\b(\w+)\b', line.split('TABLE')[-1] if 'TABLE' in line else '')
            if match:
    ...
    def _assess_additional_risks(self, line: str, line_upper: str) -> List[str]:
        ...
        if any(keyword in line_upper for keyword in ['PROD', 'PRODUCTION', 'LIVE']) or \
           any(keyword in line for keyword in ['prod', 'production', 'live']):
        ...
        if not any(keyword in line_upper for keyword in ['BEGIN', 'COMMIT', 'ROLLBACK']):
    ...
    def _adjust_risk_level(self, base_risk: RiskLevel, additional_risks: List[str]) -> RiskLevel:
        ...
        if 'def downgrade()' in content and 'pass' in content:
            ...
            for i, line in enumerate(lines):
                if 'def downgrade()' in line:
                    ...
                    downgrade_line = i
                elif downgrade_found and line.strip() and not line.strip().startswith('#'):
                    if line.strip() == 'pass':
                        issues.append(SafetyIssue(
                            risk_level=RiskLevel.HIGH,
    ...
    def _check_error_handling(self, content: str) -> List[SafetyIssue]:
        ...
        if 'try:' not in content and any(op in content.upper() for op in ['UPDATE', 'DELETE', 'INSERT']):
            issues.append(SafetyIssue(
                risk_level=RiskLevel.MEDIUM,
    ...
    def _check_data_migration_safety(self, content: str, lines: List[str]) -> List[SafetyIssue]:
        ...
        for line_num, line in enumerate(lines, 1):
            if 'UPDATE' in line.upper() or 'DELETE' in line.upper():
                if 'LIMIT' not in line.upper() and 'WHERE' in line.upper():
                    ...
                    if any(broad_condition in line.upper() for broad_condition in ['1=1', 'TRUE', 'IS NOT NULL']):
                        issues.append(SafetyIssue(
                            risk_level=RiskLevel.HIGH,
    ...
    def _check_transaction_safety(self, content: str, lines: List[str]) -> List[SafetyIssue]:
        ...
        if not has_begin and not has_commit and any(op in content.upper() for op in ['UPDATE', 'DELETE', 'INSERT']):
            issues.append(SafetyIssue(
                risk_level=RiskLevel.MEDIUM,
    ...
    def _check_migration_combinations(self, migrations: List[str]) -> List[SafetyIssue]:
        ...
        has_data_changes = any('update' in mig.lower() or 'insert' in mig.lower() or 'delete' in mig.lower()
...
class ProductionSafetyValidator:
    ...
    def __init__(self):
        ...
        self.backup_required = os.environ.get('MIGRATION_BACKUP_REQUIRED', 'true').lower() == 'true'

    def validate_production_readiness(self, migration_file: str) -> Tuple[bool, List[str]]:
        ...
        if self.required_approvals:
            pending_approvals = [approval for approval in self.required_approvals if approval.strip()]
        ...
        high_risk_issues = [issue for issue in safety_issues
                           if issue.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]]

        if high_risk_issues:
            issues.extend([f"High risk operation: {issue.message}" for issue in high_risk_issues])

        return len(issues) == 0, issues


def validate_migration_safety(migration_file: str, production_mode: bool = False) -> List[SafetyIssue]:
    """
    ...
    if production_mode:
        ...
        is_ready, prod_issues = prod_validator.validate_production_readiness(migration_file)
...
def print_safety_report(issues: List[SafetyIssue]) -> None:
    ...
    for risk_level in risk_order:
        if risk_level in risk_groups:
            emoji = {"CRITICAL": "🚨", "HIGH": "⚠️", "MEDIUM": "⚡", "LOW": "ℹ️"}[risk_level.name]
