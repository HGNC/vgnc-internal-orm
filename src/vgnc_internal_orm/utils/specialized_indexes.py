...
class UniqueCompositeIndex:
    ...
    def to_sqlalchemy_constraint(self) -> UniqueConstraint:
        ...
        return UniqueConstraint(
            *self.columns, name=self.name, deferrable=self.deferrable is not None
        )
...
class PartialIndex:
    ...
    def to_sqlalchemy_index(self) -> Index:
        ...
        # Note: MySQL 8.0+ supports filtered indexes
        return Index(
            self.name,
            *self.columns,
            unique=self.unique,
            mysql_where=self.where_condition,
        )

    def to_ddl(self) -> str:
        """Generate DDL statement (MySQL syntax)."""
        columns_str = ", ".join(self.columns)
...
class FunctionalIndex:
    ...
    def to_sqlalchemy_index(self) -> Index:
        """Convert to SQLAlchemy Index."""
        return Index(self.name, text(self.expression), unique=self.unique)
...
class SpecializedIndexManager:
    ...
    def __init__(self) -> None:
        ...
        self.functional_indexes: dict[str, list[FunctionalIndex]] = {}
    ...
    def _load_all_indexes(self) -> None:
        ...
        self.fulltext_indexes["genefams"] = (
            VGNCIndexDefinitions.GENEFAM_FULLTEXT_INDEXES
        )
        self.fulltext_indexes["genefam_orthology_group"] = (
            VGNCIndexDefinitions.ORTHOLOGY_FULLTEXT_INDEXES
        )
        ...
        self.unique_indexes["chromosomes"] = (
            VGNCIndexDefinitions.CHROMOSOMES_UNIQUE_INDEXES
        )
        self.unique_indexes["assembly"] = VGNCIndexDefinitions.ASSEMBLY_UNIQUE_INDEXES
        self.unique_indexes["genefam_orthology_group"] = (
            VGNCIndexDefinitions.ORTHOLOGY_UNIQUE_INDEXES
        )
        ...
        self.partial_indexes["chromosomes"] = (
            VGNCIndexDefinitions.CHROMOSOMES_PARTIAL_INDEXES
        )
        self.partial_indexes["assembly"] = VGNCIndexDefinitions.ASSEMBLY_PARTIAL_INDEXES
        self.partial_indexes["genefam_orthology_group"] = (
            VGNCIndexDefinitions.ORTHOLOGY_PARTIAL_INDEXES
        )

        # Functional indexes
        self.functional_indexes["species"] = (
            VGNCIndexDefinitions.SPECIES_FUNCTIONAL_INDEXES
        )
    ...
    def create_unique_composite_indexes(
        self, table_name: str
    ) -> list[UniqueCompositeIndex]:
    ...
    def generate_mysql_ddl(self, table_name: str) -> list[str]:
        ...
        # Full-text indexes (MySQL syntax)
        for ft_index in self.create_fulltext_indexes(table_name):
            columns_str = ", ".join(ft_index.columns)
            ddl = (
                f"CREATE FULLTEXT INDEX {ft_index.name} ON {table_name} ({columns_str})"
            )
    ...
    def apply_to_models(self, models: list[Any] | None = None) -> dict[str, list[str]]:
        ...
        for model in models:
            ...
            # Convert unique constraints separately
            for unique_index in self.create_unique_composite_indexes(table_name):
                specialized_constraints.append(unique_index.to_sqlalchemy_constraint())
            ...
            # Apply to model - combine indexes and constraints
            if hasattr(model, "__table_args__"):
                existing_args = (
                    list(model.__table_args__)
                    if isinstance(model.__table_args__, (list, tuple))
                    else []
                )
                for idx in specialized_indexes:
                    existing_args.append(idx)
                for constraint in specialized_constraints:
                    existing_args.append(constraint)
                model.__table_args__ = tuple(existing_args)
            else:
                # Create combined tuple of indexes and constraints
                model.__table_args__ = tuple(
                    specialized_indexes + specialized_constraints
                )

            applied_indexes[table_name] = [
                str(type(idx).__name__) for idx in specialized_indexes
            ]
    ...
    def analyze_index_usage(
        ...
        analysis: dict[str, Any] = {
            ...
            "available_indexes": {
                ...
                "unique_composite": len(
                    self.create_unique_composite_indexes(table_name)
                ),
        ...
        for query in query_patterns:
            ...
            if any(
                keyword in query_lower for keyword in ["search", "like", "contains"]
            ):
                if self.create_fulltext_indexes(table_name):
                    support_score += 3
                    matching_indexes.extend(
                        [idx.name for idx in self.create_fulltext_indexes(table_name)]
                    )
            ...
            if "distinct" in query_lower or "group by" in query_lower:
                if self.create_unique_composite_indexes(table_name):
                    ...
                    matching_indexes.extend(
                        [
                            idx.name
                            for idx in self.create_unique_composite_indexes(table_name)
                        ]
                    )

            # Check for filter patterns
            if "where" in query_lower and (
                "is_active" in query_lower or "is_" in query_lower
            ):
                if self.create_partial_indexes(table_name):
                    support_score += 2
                    matching_indexes.extend(
                        [idx.name for idx in self.create_partial_indexes(table_name)]
                    )

            analysis["query_support"][query] = {
                "support_score": support_score,
                "matching_indexes": list(matching_indexes),  # Ensure list type
            }
