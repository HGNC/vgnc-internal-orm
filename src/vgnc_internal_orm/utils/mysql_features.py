        ...
        with engine.connect() as conn:
            try:
                ...
                charset_result = conn.execute(text(
                    "SELECT VARIABLES('character_set_connection')"
                )).scalar()
                result['charset'] = charset_result

                # Check connection collation
                collation_result = conn.execute(text(
                    "SELECT VARIABLES('collation_connection')"
                )).scalar()
                ...
                server_charset = conn.execute(text(
                    "SELECT VARIABLES('character_set_server')"
                )).scalar()
            ...
            for i, char in enumerate(text):
                ...
                if UTF8MB4Handler.requires_utf8mb4(char):
                    ...
                    result['problematic_chars'].append({
                        ...
                        'description': cls._get_char_description(char_code)
                    })
...
class FullTextSearch:
    ...
    class SearchMode:
        ...
        NATURAL_LANGUAGE_WITH_EXPANSION = "IN NATURAL LANGUAGE MODE WITH QUERY EXPANSION"
    ...
    def build_match_query(
        ...
        escaped_query = FullTextSearch._escape_search_query(search_query)
...
class MySQLQueryOptimizer:
    ...
    def analyze_query_plan(
        ...
        try:
            ...
            for row in plan_result:
                plan_row = dict(row._mapping) if hasattr(row, '_mapping') else row._asdict()
    ...
    def suggest_indexes(
        ...
        try:
            ...
            existing_indexes = session.execute(text(existing_indexes_sql)).fetchall()
    ...
    @staticmethod
    def inject_hints(query: Union[str, text], hints: List[str]) -> text:
        """
    ...
    def get_slow_query_analysis(
        ...
        try:
            ...
            log_status = session.execute(text(
            ...
            for query in slow_queries:
                query_info = {
                    'sql': query.sql_text[:500] + '...' if len(query.sql_text) > 500 else query.sql_text,
                    'exec_count': query.exec_count,
                    'avg_time': query.avg_time_seconds,
                    'avg_lock_time': query.avg_lock_time_seconds,
                    'optimization_suggestions': MySQLQueryOptimizer._analyze_slow_query(query.sql_text)
                }
    ...
    @staticmethod
    def _analyze_plan_for_optimizations(plan_details: List[Dict]) -> Dict[str, Any]:
        ...
        for step in plan_details:
            ...
            # Check join types
            if 'ref' in step.get('type', '').lower() or 'eq_ref' in step.get('type', '').lower():
    ...
    @staticmethod
    def _detect_plan_warnings(plan_details: List[Dict]) -> List[str]:
        ...
        for step in plan_details:
            ...
            if 'using filesort' in extra:
                warnings.append(f"Filesort detected for table '{table_name}' - consider optimizing ORDER BY")

            if 'using temporary' in extra:
                warnings.append(f"Temporary table created for '{table_name}' - consider query rewrite")
            ...
            if rows > 100000:
                warnings.append(f"Large number of rows ({rows}) estimated for '{table_name}'")
    ...
    def _analyze_query_for_indexes(
        ...
        where_match = re.search(r'WHERE\s+(.+?)(?:\s+ORDER\s+BY|\s+GROUP\s+BY|\s+LIMIT|$)',
                              query_pattern, re.IGNORECASE | re.DOTALL)
        if where_match:
        ...
        order_match = re.search(r'ORDER\s+BY\s+(.+?)(?:\s+GROUP\s+BY|\s+LIMIT|$)',
        ...
        for join_match in join_matches:
            if join_match[0] == table_name:
                analysis['join_columns'].append(join_match[1])
            elif join_match[2] == table_name:
                analysis['join_columns'].append(join_match[3])

        # Generate index suggestions
        all_columns = set(analysis['where_columns'] +
                         analysis['order_by_columns'] +
                         analysis['group_by_columns'] +
                         analysis['join_columns'])

        if all_columns:
            # Primary suggestion: composite index covering WHERE and ORDER BY
            primary_columns = analysis['where_columns'] + analysis['order_by_columns']
            if primary_columns:
    ...
    def _analyze_slow_query(sql_text: str) -> List[str]:
        ...
        if re.search(r'WHERE.*\(.*\+.*\)', sql_upper):
            suggestions.append("Avoid calculations in WHERE clause - consider indexed columns")
    ...
    def _extract_query_patterns(analysis: Dict[str, Any]) -> None:
        ...
        for query_info in analysis['queries']:
            ...
            for table in table_matches:
                patterns['tables_used'][table] = patterns['tables_used'].get(table, 0) + 1

            # Count operations
            if 'JOIN' in sql:
                patterns['operations']['joins'] = patterns['operations'].get('joins', 0) + 1
            if 'GROUP BY' in sql:
                patterns['operations']['group_by'] = patterns['operations'].get('group_by', 0) + 1
            if 'ORDER BY' in sql:
                patterns['operations']['order_by'] = patterns['operations'].get('order_by', 0) + 1

            # Count common issues
            if 'SELECT *' in sql:
                patterns['issues']['select_star'] = patterns['issues'].get('select_star', 0) + 1
            if 'WHERE' not in sql:
                patterns['issues']['no_where'] = patterns['issues'].get('no_where', 0) + 1
