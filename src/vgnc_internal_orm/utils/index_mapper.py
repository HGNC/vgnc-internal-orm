...
from sqlalchemy import (
    ...
    CheckConstraint, text, Column, Integer, String
)
...
class IndexMapper:
    ...
    def _load_models(self) -> None:
        ...
        try:
            ...
            self.association_tables = [
                _assembly_has_chr, _gene_alt_name, _gene_alt_symbol,
                _gene_has_comment, _gene_has_flag, _gene_has_family
            ]
        except Exception:
    ...
    def analyze_current_indexes(self) -> IndexMappingResult:
        ...
        result.performance_recommendations = self._generate_performance_recommendations(result.indexes)
        result.missing_indexes = self._identify_missing_indexes(result.indexes)
        result.fulltext_opportunities = self._identify_fulltext_opportunities(result.indexes)
    ...
    def _extract_foreign_key_constraints(self, model) -> List[ConstraintDefinition]:
    ...
    def _extract_unique_constraints(self, model) -> List[ConstraintDefinition]:
                ...
                for item in table_args:
                    if isinstance(item, UniqueConstraint):
                        ...
                        constraint_def = ConstraintDefinition(
                            name=item.name or f"uq_{model.__tablename__}_{'_'.join(columns)}",
                            table_name=model.__tablename__,
    ...
    def _extract_indexes(self, model) -> List[IndexDefinition]:
    ...
    def _extract_association_primary_key(self, association) -> List[str]:
        """Extract primary key columns from an association table."""
        pk_columns = [col.name for col in association.primary_key.columns]
        return pk_columns

    def _extract_association_foreign_keys(self, association) -> List[ConstraintDefinition]:
    ...
    def _generate_performance_recommendations(self, indexes: Dict[str, List[IndexDefinition]]) -> List[str]:
        ...
        for table_name, table_indexes in indexes.items():
            if not table_indexes:
                recommendations.append(f"Table '{table_name}' has no indexes - consider adding indexes for frequently queried columns")
        ...
        for table_name, table_indexes in indexes.items():
            ...
            if table_name in ['chromosomes', 'assembly']:
                if 'species_id' not in indexed_columns:
                    recommendations.append(f"Consider adding index on '{table_name}.species_id' for foreign key performance")

        return recommendations

    def _identify_missing_indexes(self, indexes: Dict[str, List[IndexDefinition]]) -> List[Dict[str, Any]]:
    ...
    def _identify_fulltext_opportunities(self, indexes: Dict[str, List[IndexDefinition]]) -> List[Dict[str, Any]]:
        ...
        for model in self.models:
            ...
            for fk in fk_constraints:
                if fk.columns:  # Only create index for single-column FKs
                    fk_index = Index(
                        f"idx_{table_name}_{'_'.join(fk.columns)}",
                        *fk.columns
                    )
