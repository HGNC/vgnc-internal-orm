...
class IndexDefinitions:
    ...
    SPECIES_INDEXES = [
        ...
        Index("idx_species_vgnc_prefix", "vgnc_prefix"),
        Index("idx_species_taxon_id", "taxon_id"),

        # Taxonomic classification indexes
        Index("idx_species_genus", "genus"),
        Index("idx_species_species", "species"),
        Index("idx_species_class", "class_name"),
        Index("idx_species_order", "order_name"),
        Index("idx_species_family", "family_name"),

        # Status and feature indexes
        Index("idx_species_is_active", "is_active"),
        Index("idx_species_is_model_organism", "is_model_organism"),
        Index("idx_species_has_reference_genome", "has_reference_genome"),

        # Composite indexes for common query patterns
        Index("idx_species_taxon_class", "taxon_id", "class_name"),
        Index("idx_species_class_order", "class_name", "order_name"),
        Index("idx_species_model_active", "is_model_organism", "is_active"),

        # External reference indexes
        Index("idx_species_assembly_accession", "assembly_accession"),

        # Full-text search indexes (defined separately for MySQL)
        # Index("fti_species_names", "scientific_name", "common_name",
    ...
    CHROMOSOME_INDEXES = [
        ...
        Index("idx_chromosomes_assembly_chromosome", "assembly_name", "chromosome_name"),
    ...
    GENEFAM_SPECIES_ENHANCED_INDEXES = [
        ...
        # Composite indexes for complex queries
        Index("idx_genefam_species_enhanced_gf_confidence", "genefam_id", "confidence_score"),
        Index("idx_genefam_species_enhanced_sp_confidence", "species_id", "confidence_score"),
        Index("idx_genefam_species_enhanced_primary_confidence", "is_primary", "confidence_score"),
    ]
    ...
    GENEFAM_ORTHOLOGY_GROUP_INDEXES = [
        ...
        # Composite indexes
        Index("idx_genefam_orthology_group_conservation_active", "conservation_level", "is_active"),
        Index("idx_genefam_orthology_group_scope_confidence", "phylogenetic_scope", "confidence_score"),
    ]
    ...
    GENEFAM_ORTHOLOGY_GROUP_MEMBERS_INDEXES = [
        ...
        # Composite indexes for complex queries
        Index("idx_genefam_orthology_members_group_representative", "group_id", "is_representative"),
        Index("idx_genefam_orthology_members_genefam_species", "genefam_id", "species_id"),
        Index("idx_genefam_orthology_members_confidence_date", "membership_confidence", "date_added"),
    ]
    ...
    SPECIES_RELATIONSHIP_INDEXES = [
        ...
        Index("idx_species_relationship_divergence_type", "divergence_time_mya", "relationship_type"),
        Index("idx_species_relationship_similarity_active", "genome_similarity", "is_active"),
    ]
...
class ConstraintDefinitions:
    ...
    CHROMOSOME_UNIQUE_CONSTRAINTS = [
        UniqueConstraint("species_id", "chromosome_name", name="uq_chromosomes_species_chromosome"),
    ]
    ...
    SPECIES_CHECK_CONSTRAINTS = [
        ...
        CheckConstraint("vgnc_prefix ~ '^[A-Z]{2,4}$'", name="ck_species_vgnc_prefix_format"),
    ]

    GENEFAM_CHECK_CONSTRAINTS = [
        CheckConstraint("gene_count >= 0", name="ck_genefams_gene_count_non_negative"),
        CheckConstraint("species_count >= 0", name="ck_genefams_species_count_non_negative"),
    ]
    ...
    ASSEMBLY_CHECK_CONSTRAINTS = [
        ...
        CheckConstraint("assembly_version ~ '^[0-9]+\\.[0-9]+$'", name="ck_assemblies_version_format"),
    ]
    ...
    ORTHOLOGY_CHECK_CONSTRAINTS = [
        ...
        CheckConstraint("membership_confidence >= 0 AND membership_confidence <= 1",
                       name="ck_orthology_membership_confidence_range"),
        CheckConstraint("divergence_time_mya >= 0", name="ck_orthology_divergence_time_positive"),
    ]
...
def get_all_constraints() -> Dict[str, List]:
    ...
    return {
        ...
        "assembly": (
            ConstraintDefinitions.ASSEMBLY_CHECK_CONSTRAINTS
        ),
...
class PerformanceIndexSets:
    ...
    GENEFAM_BROWSING_INDEXES = [
        ...
        # Combined filters
        Index("idx_genefam_browse_type_category", "family_type", "functional_category", "is_active"),
    ]
    ...
    ORTHOLOGY_ANALYSIS_INDEXES = [
        # Group-based queries
        Index("idx_orthology_group_analysis", "group_id", "conservation_level", "is_active"),
        Index("idx_orthology_member_analysis", "group_id", "membership_confidence", "is_representative"),

        # Species-based queries
        Index("idx_orthology_species_analysis", "species_id", "group_id", "membership_confidence"),
        Index("idx_orthology_relationship_analysis", "species_a_id", "species_b_id", "relationship_type", "is_active"),
    ]
    ...
    ASSEMBLY_QUERY_INDEXES = [
        ...
        Index("idx_assembly_species_quality", "species_id", "is_primary", "is_reference"),
...
class IndexValidator:
    ...
    def validate_index_set(indexes: List[Index], table_name: str) -> Dict[str, Any]:
        ...
        overlapping = [
            cols for cols in column_sets
            if column_sets.count(cols) > 1
        ]
    ...
    @staticmethod
    def analyze_index_usage_pattern(indexes: List[Index], common_queries: List[str]) -> Dict[str, Any]:
        ...
        analysis["query_coverage"] = {
            "SELECT * FROM species WHERE scientific_name = ?": "covered" if "scientific_name" in indexed_columns else "missing",
            "SELECT * FROM genefams WHERE family_type = ?": "covered" if "family_type" in indexed_columns else "missing",
        }
