...
class TestBasicQueryPerformance:
    ...
    def test_simple_species_query(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "simple species query"
        )

    def test_species_list_query(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "species list query"
        )
        assert len(result) >= 5  # Should have at least 5 species from fixtures

    def test_species_filter_query(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def filter_species(session):
            return (
                session.query(Species).filter(Species.genefam_prefix == "HSA").first()
            )

        result = benchmark(filter_species, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "species filter query"
        )
        assert result.genefam_prefix == "HSA"

    def test_species_count_query(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "species count query"
        )
        assert result >= 5

    def test_assembly_by_species_query(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def query_assemblies(session):
            return session.query(Assembly).filter(Assembly.taxon_id == 9606).all()

        result = benchmark(query_assemblies, session)
        assert_performance_threshold(
            benchmark,
            performance_thresholds["simple_query"],
            "assembly by species query",
        )
        assert len(result) >= 1

    def test_chromosomes_by_species_query(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def query_chromosomes(session):
            return session.query(Chromosomes).filter(Chromosomes.taxon_id == 9606).all()

        result = benchmark(query_chromosomes, session)
        assert_performance_threshold(
            benchmark,
            performance_thresholds["simple_query"],
            "chromosomes by species query",
        )
...
class TestComplexQueryPerformance:
    ...
    def test_species_with_multiple_filters(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["complex_query"], "complex species filter"
        )
        assert len(result) >= 1

    def test_or_query_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["complex_query"], "OR query"
        )
        assert len(result) == 3
    ...
    def test_join_query_performance(
        ...
        def join_query(session):
            return (
                session.query(Species, Assembly)
                .join(Assembly, Species.taxon_id == Assembly.taxon_id)
                .all()
            )

        result = benchmark(join_query, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["complex_query"], "join query"
        )
        assert len(result) >= 5  # Should have species-assembly pairs

    def test_subquery_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def subquery_query(session):
            ...
            return (
                session.query(Species)
                .filter(Species.taxon_id.in_(species_with_assemblies))
                .all()
            )

        result = benchmark(subquery_query, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["complex_query"], "subquery"
        )
        assert len(result) >= 5

    def test_order_by_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def order_by_query(session):
            return session.query(Species).order_by(Species.display_name.asc()).all()

        result = benchmark(order_by_query, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "ORDER BY query"
        )
        assert len(result) >= 5

    def test_limit_offset_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "LIMIT/OFFSET query"
        )
        assert len(result) <= 3
...
class TestAggregateQueryPerformance:
    ...
    def test_count_by_group(
        ...
        def count_by_group(session):
            return (
                session.query(Species.is_live, func.count(Species.taxon_id))
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["aggregate_query"], "COUNT BY GROUP BY"
        )
        assert len(result) >= 1
    ...
    def test_sum_aggregate(
        ...
        def sum_query(session):
            ...
            return (
                ...
                .join(Chromosomes, Species.taxon_id == Chromosomes.taxon_id)
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["aggregate_query"], "SUM aggregate"
        )
        assert len(result) >= 5
    ...
    def test_max_aggregate(
        ...
        def max_query(session):
            return session.query(func.max(Assembly.id)).scalar()

        result = benchmark(max_query, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["aggregate_query"], "MAX aggregate"
        )
    ...
    def test_having_clause(
        ...
        def having_query(session):
            return (
                ...
                .join(Chromosomes, Species.taxon_id == Chromosomes.taxon_id)
                .group_by(Species.taxon_id)
                .having(func.count(Chromosomes.chr_id) > 10)
                .all()
            )

        benchmark(having_query, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["aggregate_query"], "HAVING clause"
        )
    ...
    def test_multiple_aggregates(
        ...
        def multiple_aggregates(session):
            return session.query(
                ...
                func.count(func.distinct(Species.genefam_prefix)).label(
                    "unique_prefixes"
                ),
                func.count(Species.taxon_id)
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["aggregate_query"], "multiple aggregates"
        )
...
class TestBulkOperationPerformance:
    ...
    def test_bulk_insert_performance(
        ...
        benchmark,
        benchmark_session,
        benchmark_data_factory,
        performance_thresholds,
    ):
        """Benchmark bulk insert performance."""
        session = benchmark_session
        data_factory = benchmark_data_factory  # Get the factory

        def bulk_insert():
            # Clean up any existing test data first
            session.query(Species).filter(
                Species.taxon_id >= 90000, Species.taxon_id < 91000
            ).delete()
            session.commit()

            # Generate test data
            test_data = data_factory["species"](1000)
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["bulk_insert"], "bulk insert"
        )
        assert result == 1000
    ...
    def test_bulk_delete_performance(
        ...
        def bulk_delete():
            # Clean up any existing test data first
            session.query(Species).filter(Species.genefam_prefix.like("TEST%")).delete()
            session.commit()

            # First insert test data
            test_data = data_factory["species"](500)
            instances = [Species(**data) for data in test_data]
            session.add_all(instances)
            session.commit()
            ...
            deleted_count = (
                session.query(Species)
                .filter(Species.genefam_prefix.like("TEST%"))
                .delete()
            )
            session.commit()
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["bulk_insert"], "bulk delete"
        )  # Use same threshold
        assert result == 500
    ...
    def test_bulk_update_performance(
        ...
        def bulk_update(session):
            ...
            updated_count = (
                session.query(Species)
                .filter(Species.taxon_id.in_([9606, 10090, 10116]))
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "bulk update"
        )
        assert result == 3
    ...
    def test_lazy_loading_performance(
        ...
        def lazy_loading(session):
            ...
            assemblies = (
                ...
                .filter(Assembly.taxon_id == species.taxon_id)
                .all()
            )
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["relationship_loading"], "lazy loading"
        )
        assert result >= 1

    def test_joined_loading_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def joined_loading(session):
            ...
            species = (
                ...
                .filter(Species.taxon_id == 9606)
                .first()
            )

            return len(species.assemblies) if hasattr(species, "assemblies") else 0

        benchmark(joined_loading, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["relationship_loading"], "joined loading"
        )
        # Note: This will return 0 because the relationship isn't properly defined in the model

    def test_selectin_loading_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def selectin_loading(session):
            ...
            species = session.query(Species).filter(Species.taxon_id == 9606).all()

            # Then load assemblies for all species
            for sp in species:
                (session.query(Assembly).filter(Assembly.taxon_id == sp.taxon_id).all())
        ...
        assert_performance_threshold(
            benchmark,
            performance_thresholds["relationship_loading"],
            "selectin loading",
        )
        assert result == 1
...
class TestIndexPerformance:
    ...
    def test_primary_key_lookup_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "primary key lookup"
        )
        assert len(result) >= 5

    def test_foreign_key_lookup_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def fk_lookup(session):
            ...
            assemblies = session.query(Assembly).filter(Assembly.taxon_id == 9606).all()
            return assemblies

        result = benchmark(fk_lookup, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "foreign key lookup"
        )
        assert len(result) >= 1

    def test_indexed_filter_performance(
        self, benchmark, populated_benchmark_db, performance_thresholds
    ):
        ...
        def indexed_filter(session):
            ...
            species = (
                session.query(Species).filter(Species.genefam_prefix == "HSA").all()
            )
            return species

        result = benchmark(indexed_filter, session)
        assert_performance_threshold(
            benchmark, performance_thresholds["simple_query"], "indexed filter"
        )
        assert len(result) == 1
